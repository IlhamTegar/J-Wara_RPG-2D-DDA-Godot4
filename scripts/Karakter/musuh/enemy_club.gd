...
func _ready() -> void:
	...
	# 🛠️ SECURITY PATCH: Sambungkan sinyal hurtbox otomatis agar tidak kelewat di Editor
...
func _on_enemy_frame_changed() -> void: 
	...
	if "attack" in nama_anim_kecil and (animated_sprite.frame == 5 or animated_sprite.frame == 6): 
		if has_node("EnemyHitbox/CollisionShape2D"):
